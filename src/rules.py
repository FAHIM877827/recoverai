# Failure-reason -> action/channel mapping lives in policy.py (POLICIES dict).
# classify() below is a thin wrapper around policy.get_policy(); it has no
# mapping of its own so there's exactly one place this logic can drift.
...
def apply_policy_gateway(df):
    ...
    Returns a copy of df with 'action' overridden where a rail fires, plus
    a new 'policy_note' column explaining why (empty string if no override).
    """
